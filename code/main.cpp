...
DEFINE_double(quality_level, 0.01, "quality level");
DEFINE_double(min_distance, 3, "min distance");
DEFINE_int32(winSizePyramid, 21, "window kernel size pyramid");
DEFINE_int32(nLevel, 3, "nLevel");

DEFINE_double(max_diff_distance, 1, "max diff distance");
DEFINE_int32(max_corners, 600, "max corners");
DEFINE_int32(interval, 5, "interval");
...
int main(int argc, char** argv){
    ...
    TrackingSetting trackSetting;
    trackSetting.kltSetting.quality_level = FLAGS_quality_level;
    trackSetting.kltSetting.min_distance = FLAGS_min_distance;
    trackSetting.kltSetting.winSizePyramid = FLAGS_winSizePyramid;
    trackSetting.kltSetting.nLevel = FLAGS_nLevel;

    trackSetting.ofSetting.max_diff_distance = FLAGS_max_diff_distance;
    trackSetting.ofSetting.max_corners = FLAGS_max_corners;
    trackSetting.ofSetting.interval = FLAGS_interval;

    //tracking
    FeatureTracks trackMatrix;
    printf("Computing track matrix...\n");
    Tracking::genTrackMatrix(images, trackMatrix, FLAGS_tWindow, FLAGS_stride, trackSetting);
    printf("Done\n");
...
void importVideo(const std::string& path, std::vector<cv::Mat>& images, double& fps, int& vcodec){
    ...
    vcodec = (int)cap.get(CAP_PROP_FOURCC);
