    ...
    namespace Tracking{
        void genTrackMatrix(const std::vector<cv::Mat>& images, FeatureTracks& trackMatrix, const int tWindow, const int stride, const TrackingSetting &trackSetting){
            ...
            for(auto v=0; v<grays.size(); ++v)
                cvtColor(images[v], grays[v], COLOR_BGR2GRAY);
            ...
            for(auto v=0; v<images.size(); ++v) {
                cv::buildOpticalFlowPyramid(grays[v], pyramid[v], cv::Size(trackSetting.kltSetting.winSizePyramid, trackSetting.kltSetting.winSizePyramid), trackSetting.kltSetting.nLevel);
            }
            ...
            for(auto v=0; v<images.size()-tWindow; v++){
                ...
                cv::goodFeaturesToTrack(grays[v], corners, trackSetting.ofSetting.max_corners, trackSetting.kltSetting.quality_level, trackSetting.kltSetting.min_distance);
                vector<cv::Point2f> newcorners;
                newcorners.reserve(corners.size());
                for(auto cid=0; cid < corners.size(); ++cid){
                    bool is_new = true;
                    if(v % trackSetting.ofSetting.interval != 0) {
                        for (auto j = 0; j < trackMatrix.tracks.size(); ++j) {
                            if (trackMatrix.tracks[j].size() + trackMatrix.offset[j] >= v) {
                                ...
                                if (dis <= trackSetting.ofSetting.max_diff_distance)
                                    is_new = false;
                            }
                        }
                    }
                ...
                for(auto i=v+1; i<images.size(); ++i){
                    ...
                    vector<uchar> status;
                    Mat err;
                    cv::calcOpticalFlowPyrLK(pyramid[i-1], pyramid[i], newcorners, tracked, status, err, cv::Size(trackSetting.kltSetting.winSizePyramid, trackSetting.kltSetting.winSizePyramid));
